(ns sabaki.aes
  (require [sabaki.galois :refer :all]))

(def s-box (hash-map 0x00 0x63 0x01 0x7c 0x02 0x77 0x03 0x7b 0x04 0xf2 0x05 0x6b 0x06 0x6f 0x07 0xc5
...
(def inv-s-box (clojure.set/map-invert s-box))

(def rcon [[0x01 0x00 0x00 0x00]
           [0x02 0x00 0x00 0x00]
           [0x04 0x00 0x00 0x00]
           [0x08 0x00 0x00 0x00]
           [0x10 0x00 0x00 0x00]
...
(defn inv-mix-columns [block]
  ...
  (let [inv-mix [gx0E gx0B gx0D gx09]]
    (->> block
         ...
         (map (partial map (fn [f v] (f v)) inv-mix))
         (partition 4)
         (map (partial map (fn [x] (reduce bit-xor x))))
         )
    )
  )
